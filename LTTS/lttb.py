...
class LTTB:
	...
	def step (self,apicalFactor,basalFactor=0):
		self.t += 1
		t = self.t
		...
		self.Isoma[:,t] = self.w@self.S_filt[:,t] + self.h + self.j_in@self.I_clock[:,t] + self.sigma_basal_cont*self.j_basal_cont@self.cont + self.S_wind[:,t]*20 +self.j_targ@self.y_targ[:,t]*basalFactor - self.b*self.W[:,t]
